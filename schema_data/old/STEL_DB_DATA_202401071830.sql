...
CREATE
DEFINER = 'root'@'%'
FUNCTION specimenExists (idSample int UNSIGNED, idOperator int UNSIGNED, start datetime, enviromental varchar(7))
...
BEGIN
  ...
  AND ts.operator = idOperator
  AND ts.start = start
  AND ts.enviromental = enviromental)
  LIMIT 1;
